...
class FactionKeywordExpansionTest {
    ...
    void everyFactionHasThreeSelectableCapitalsOutsideTheDeckPool() {
        ...
        PrototypeCardPool pool = new PrototypeCardPool();
        assertEquals(18, roster.all().size());
        for (String faction : FactionDecks.FACTIONS) {
    ...
    void chosenCapitalIsDeployedAndMustMatchSingleFactionDeck() {
        ...
        List<CardDefinition> ares = new FactionDecks(matches.pool()).starter("ARES");
        CardDefinition zeusCapital = matches.capitals().require("zeus_capital_keraunos_spire");
        var state = matches.create(99L, zeus, ares, zeusCapital,
                matches.capitals().require("ares_capital_red_citadel"));
        ...
        assertThrows(IllegalArgumentException.class, () -> matches.create(99L, zeus, ares,
                matches.capitals().require("hades_capital_styx_gate"),
                matches.capitals().require("ares_capital_red_citadel")));
    }

...
class AlliedDeckTest {
    ...
    @Test void rejectsWrongCapitalThirdFactionAndMultipleAllies(){
        ...
        assertThrows(IllegalArgumentException.class,()->new DeckBuild("bad","ZEUS","POSEIDON",factory.capitals().forFaction("ARES").get(0),good.cards()));
        List<CardDefinition> bad=new ArrayList<>(good.cards());bad.add(factory.pool().cardsForFaction("ARES").get(0));
    ...
    @Test void legalBotMatchesUseAlliedDeckOnHexBoard(){
        DeckBuild human=mixed();DeckBuild bot=new DeckBuild("Bot","ARES",null,factory.capitals().forFaction("ARES").get(0),new FactionDecks(factory.pool()).starter("ARES"));

...
public final class BotPlayer {
    ...
    private int capitalSynergy(GameState state, int playerId, String action) {
        ...
        return switch (passive) {
            case STORM_TITHE, FERRY_TOLL -> action.equals("cast") || action.equals("react") ? 8 : 0;
            case TRIDENT_RESTORATION -> action.equals("play") ? 3 : 0;
            case DEEP_RESERVES -> action.equals("burrow") ? 8 : 0;
            case BLOODLUST -> action.equals("attack") ? 6 : 0;
            case WAR_CAMP_DRILL -> action.equals("play") ? 4 : 0;
            case RELENTLESS_ADVANCE -> action.equals("move") ? 5 : 0;
            case CLOUDWARD -> action.equals("blink") ? 5 : 0;
            case FORGE_EFFICIENCY -> action.equals("play") ? 4 : 0;
            default -> 0;
        };
    }

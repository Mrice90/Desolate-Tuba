...
All 18 Capitals have unique painterly environment illustrations built for their name, faction, and strategic identity. The painted Character rollout begins with a six-card Zeus benchmark spanning mobility, defense, magic, ranged combat, creatures, and apex scale. The remaining deck-building cards use deterministic illustrations combining faction environments, card type, name-derived symbols, mechanics, and stable seeded composition; missing painted resources safely fall back to that renderer. See [the card-art system](docs/card-art-system.md) for its visual vocabulary and asset provenance.

...
All 18 Capitals have unique painterly environment illustrations built for their name, faction, and strategic identity. Every playable Zeus card now has a unique painted illustration across Characters, Spells, Lands, and Structures. The Poseidon rollout has begun with unique art for its first 12 Characters (costs 1–4); cards still awaiting bespoke art use deterministic illustrations combining faction environments, card type, name-derived symbols, mechanics, and stable seeded composition. Missing painted resources safely fall back to that renderer. See [the card-art system](docs/card-art-system.md) for its visual vocabulary and asset provenance.
